...
for rec_no = 1 : length(up.pt_id)
    ...
    while ~feof(fileID)
        ...
        elseif contains(curr_text_line, '_') && ~contains(curr_text_line, 'layout')
        ...
        elseif contains(curr_text_line, '~') && ~contains(curr_text_line, 'layout')
...
for pt_stay_no = 1 : length(up.pt_id)
    ...
    while ~feof(fileID)
        ...
        if contains(curr_text_line, 'Location')
            fix.loc = curr_text_line(strfind(curr_text_line, ': ')+2:end);
        end
        if contains(curr_text_line, 'age')
        ...
        if contains(curr_text_line, 'sex')
...
for file_no = 1 : length(created_files)
    ...
    for curr_var = {'fs', 'header', 'data'}
        switch curr_var{1,1}
            ...
            case 'header'
                [created_var,c1,~] = intersect(created_data.header, up.extraction.rel_sigs);
                [ref_var,r1,~] = intersect(ref_data.header, up.extraction.rel_sigs);
            case 'data'
...
rel_line = ~cellfun(@isempty, strfind(temp.top_line, ':'));
